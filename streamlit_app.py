...
def video_frame_callback(frame):
    ...
    # Debugging: Check the frame size
    if frame_size is None:
        frame_size = (mirrored.shape[1], mirrored.shape[0])
        st.write(f"Frame size determined: {frame_size}")

    # Initialize the VideoWriter once we know the frame size
    if recording and video_writer is None:
        st.write("Initializing VideoWriter...")
        video_writer = cv2.VideoWriter(output_filename, cv2.VideoWriter_fourcc(*'XVID'), fps, frame_size)
        
        # Check if VideoWriter was successfully created
        if video_writer is None or not video_writer.isOpened():
            st.error("Failed to initialize VideoWriter")
        else:
            st.write("VideoWriter initialized successfully")
...
def main():
    ...
    if recording:
        ...
        if video_writer is not None:
            st.write("Releasing VideoWriter and saving video...")
            video_writer.release()
            video_writer = None
            st.success(f"Video saved as {output_filename}")
        else:
            st.write("VideoWriter was never initialized")
